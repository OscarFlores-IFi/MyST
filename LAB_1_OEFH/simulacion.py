...
class Optimizacion:
    ...
    def simulacion(csv, ndias, model_close, Ud, cetes):
        ...
        for j in data:
            ven = []
            for i in ndias:
                ven.append(crear_ventanas(j['Close'].dropna(),
                                          i))  # IMPORTANTE!! Se asume que las bases de datos siempre recibiran el nombre de una columna 'Close'
            vent.append(ven)
...
cetes, ndias, model_close, l_vec, n_vec, iteraciones, C, rf, nombre):
        ...
        for cic in range(iteraciones):
            ...
            decisiones = np.concatenate(
                (decisiones, padres))  # agregamos los 'padres' de las nuevas generaciones a la lista.
        ...
        pickle.dump([punt, padres, hist_mean, hist_std, hist_cal, hist_padres],
                    open(nombre + '.sav', 'wb'))  # guarda las variables más importantes al finalizar.
